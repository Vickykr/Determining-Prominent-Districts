...
int32_t main()
{
	...
	//freopen("Andhra_Pradesh.txt","r",stdin);
	//freopen("Andhra_Pradesh.txt","r",stdin);
	//freopen("Andhra_Pradesh_out.txt","w",stdout);
	...
	for(i=0;i<n;i++)//mapping nodes using numbers
	{
		cin>>s;
		mp[s]=i;
		mpr[i]=s;
	...
	int mx=max_element(BC,BC+n+1)-BC;//Finding prominent using maximum betweenness centrality
	cout<<"			"<<state<<"				"<<endl<<endl;
	cout<<"Most prominent distric in "<<state<<" is "<<mpr[mx]<<endl<<endl;
	cout<<"Betweenness centrality of all disticts of "<<state<<" are :"<<endl<<endl;
	for(i=0;i<n;i++)
	cout<<mpr[i]<<" "<<BC[i]<<endl;
}
